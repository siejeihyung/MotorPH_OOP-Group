    ...
    public List<Employee> findAllEmployees() {
        ...
        for (String[] row : data) {
            try {
                ...
                double rice = Double.parseDouble(row[14].replace(",", ""));
                double phone = Double.parseDouble(row[15].replace(",", ""));
                double clothing = Double.parseDouble(row[16].replace(",", ""));
                
                // Composition
                Benefits benefitsObj = new Benefits(rice, phone, clothing);

                employeeList.add(new RegularEmployee(id, lName, fName, basic, semi, hourly, benefitsObj));
            } catch (Exception e) {
